...
std::map<int, motorState> MotorDriver::getMotorState(
    const std::vector<int>& state_motor_ids) {
  std::map<int, motorState> motor_state_map;

  motorState state;
  for (int motor_id : state_motor_ids) {
    // TODO: Add check that enable motor id is in initialized motor_ids_ vector
    // using std::find
    motor_CAN_interface_.sendCANFrame(motor_id, default_msgs::motorStateMsg);
    usleep(motorReplyWaitTime);
    if (motor_CAN_interface_.receiveCANFrame(CAN_reply_msg_)) {
      state = decodeCANFrame(CAN_reply_msg_);
      is_motor_enabled_[motor_id] = true;
    } else {
      perror("MotorDriver::getMotorState() Unable to Receive CAN Reply.");
    }

    if (motor_id != state.motor_id)
      perror(
          "MotorDriver::getMotorState() Received message does not have the same "
          "motor id!!");
